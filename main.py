...
def match_rj(rj_code):
    ...
    try:
        r = s.get(url, allow_redirects=False, cookies=R_COOKIE, proxies=proxies)
        # HTTP状态码==200表示请求成功
...
def match_rt(rt_code):
    ...
    try:
        r = s.get(url + '.html', allow_redirects=False, cookies=R_COOKIE, proxies=proxies)
        if r.status_code != 200:
            # print("    Status code:", r.status_code, "\nurl:", url)
            return r.status_code, "", "", []
...
root.title('DLsite重命名工具 v' + VERSION)  # 给窗口的可视化起名字
...
try:
    ...
    with open(fname, "w", encoding='utf-8') as f:
        json.dump({'template': '', 'replace_rules': [], 'proxy': ''}, f, ensure_ascii=False, sort_keys=True,
                indent=4, separators=(',', ': '))
    text.insert(tk.END, "**使用默认命名模板:\n")
    text.insert(tk.END, "  workno [circle] title (cv)\n")
...
btn = tk.Button(root, text='选择路径', command=lambda: thread_it(change_name))
